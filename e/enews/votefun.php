...
function AddInfoVote($classid,$id,$vote){
	...
	$r=$empire->fetch1("select id,voteip,votetext,voteclass,doip,dotime from {$dbtbpre}enewsinfovote where id='$id' and classid='$classid' limit 1");
	if(empty($r['id'])||empty($r['votetext']))
	{
		printerror("NotVote","history.go(-1)",1);
	}
	$re=DoVote($r,$vote);
	$sql=$empire->query("update {$dbtbpre}enewsinfovote set votetext='".addslashes($re['votetext'])."',voteip='$re[voteip]' where id='$id' and classid='$classid' limit 1");
	$usql=$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']." set votenum=votenum+".$re['votetotal']." where id='$id' and classid='$classid'");
	if($sql)
	{
