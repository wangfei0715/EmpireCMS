...
function EditCjNews($add,$newstext,$userid,$username){
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		if($dofield=="morepic")
		{
			$add[$dofield]=ReturnMorepicpath($add['msmallpic'],$add['mbigpic'],$add['mpicname'],$add['mdelpicid'],$add['mpicid'],$add,$add['mpicurl_qz'],1);
		}
		//下载地址
		if($dofield=="downpath")
		{
		...
		if($dofield=="newstime")
		{continue;}
		$update.=",".$dofield."='".addslashes($add[$dofield])."'";
	}
	$sql=$empire->query("update {$dbtbpre}ecms_infotmp_".$cr[tbname]." set keyboard='".addslashes($add[keyboard])."',newstime='$add[newstime]'".$update." where id='$add[id]'");
	if($sql)
	{
		//操作日志
...
function GetCjMorepicpath($text,$text1,$text2,$exp,$exp1,$exp2,$url){
	...
	$down=ReturnMorepicpath($smallpic,$bigpic,$picname,$delpicid,$picid,$add,$downurl,0);
	return $down;
}
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	...
	while($nr=$empire->fetch($sql))
	{
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			{
				//标题去除html代码
		        $zzvalue=htmlspecialchars(strip_tags(str_replace("\r\n","",$zzvalue)));
				if(empty($zzvalue))
				{
					$next=1;
					break;
