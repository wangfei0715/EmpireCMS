...
function LoadTempGroup($add,$userid,$username){
	...
	$loadtemptext=$r['gname'].$pageexp.$bqtemp.$pageexp.$jstemp.$pageexp.$listtemp.$pageexp.$newstemp.$pageexp.$pubtemp.$pageexp.$searchtemp.$pageexp.$tempvar.$pageexp.$votetemp.$pageexp.$classtemp.$pageexp.$pltemp.$pageexp.$printtemp.$pageexp.$pagetemp;
...
function LoadInTempChangeChar($tempchar,$text){
	...
	if($tempchar=='GB2312')//简体GB2312
	{
		...
		elseif($phome_ecms_charver=='big5')
		{
			$text=DoIconvVal("GB2312","BIG5",$text,1);
	...
	{
		if($phome_ecms_charver=='utf-8'&&EmpireCMS_CHARVER=='UTF-8')
		{
			return $text;
		}
		if($phome_ecms_charver=='gb2312')
		...
		elseif($phome_ecms_charver=='utf-8')//繁体
		{
			$text=DoIconvVal("UTF8","GB2312",$text,1);
			$text=DoIconvVal("GB2312","BIG5",$text);
			$text=DoIconvVal("BIG5","UTF8",$text);
	...
	elseif($tempchar=='BIG5')//繁体BIG5
	{
		...
		elseif($phome_ecms_charver=='utf-8')
		{
	...
	{
		if($phome_ecms_charver=='utf-8'&&EmpireCMS_CHARVER=='TC-UTF-8')
		{
			return $text;
		}
		if($phome_ecms_charver=='gb2312')
		...
		elseif($phome_ecms_charver=='utf-8')//简体
		{
			$text=DoIconvVal("UTF8","BIG5",$text,1);
			$text=DoIconvVal("BIG5","GB2312",$text);
			$text=DoIconvVal("GB2312","UTF8",$text);
...
function LoadInTGPubtemp($gid,$en,$record,$field,$text,$isold=0){
	...
	$sql=$empire->query("insert into ".$tb."(id,indextemp,cptemp,searchtemp,searchjstemp,searchjstemp1,otherlinktemp,downsofttemp,onlinemovietemp,listpagetemp,gbooktemp,loginiframe,otherlinktempsub,otherlinktempdate,loginjstemp,downpagetemp,pljstemp,schalltemp,schallsubnum,schalldate) values('$r[0]','".addslashes($r[1])."','".addslashes($r[3])."','".addslashes($r[4])."','".addslashes($r[5])."','".addslashes($r[6])."','".addslashes($r[7])."','".addslashes($r[9])."','".addslashes($r[10])."','".addslashes($r[11])."','".addslashes($r[12])."','".addslashes($r[13])."','$r[14]','$r[15]','".addslashes($r[16])."','".addslashes($r[17])."','".addslashes($r[18])."','".addslashes($r[19])."','$r[20]','$r[21]');");
	//5.1以下版本
	if($isold==1&&$r[2])
	{
	...
	if(($isold==1||$isold==2)&&$r[8])
	{
		...
		$empire->query("insert into ".$printtb."(tempid,tempname,temptext,isdefault,showdate,modid) values(NULL,'".addslashes($printtempname)."','".addslashes($r[8])."',1,'Y-m-d H:i:s',1);");
	}
}
...
function LoadInTGPrinttemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$empire->query("insert into ".$tb."(tempid,tempname,temptext,isdefault,showdate,modid) values('$r[0]','".addslashes($r[1])."','".addslashes($r[2])."','$r[3]','".addslashes($r[4])."','$r[5]');");
	}
}
...
function AddEBakTemp($temptype,$gid,$tempid,$tempname,$temptext,$subnews,$isdefault,$listvar,$rownum,$modid,$showdate,$subtitle,$classid,$docode,$userid,$username){
	...
	$docode=(int)$docode;
	$baktime=time();
	$empire->query("insert into {$dbtbpre}enewstempbak(tempid,tempname,temptext,subnews,isdefault,listvar,rownum,modid,showdate,subtitle,classid,docode,baktime,temptype,gid,lastuser) values('$tempid','".addslashes($tempname)."','".addslashes($temptext)."','$subnews','$isdefault','".addslashes($listvar)."','$rownum','$modid','".addslashes($showdate)."','$subtitle','$classid','$docode','$baktime','$temptype','$gid','$username');");
}
...
function ReEBakTemp($add,$userid,$username){
	...
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsbqtemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',listvar='".StripAddsData($r[listvar])."',subnews='$r[subnews]',rownum='$r[rownum]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='classtemp')//封面模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsclasstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',classid='$r[classid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='jstemp')//JS模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsjstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',classid='$r[classid]',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]',subnews='$r[subnews]',subtitle='$r[subtitle]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='listtemp')//列表模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewslisttemp",$gid)." set subnews='$r[subnews]',tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',listvar='".StripAddsData($r[listvar])."',rownum='$r[rownum]',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',subtitle='$r[subtitle]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='newstemp')//内容模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsnewstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]',classid='$r[classid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='pltemp')//评论模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspltemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='printtemp')//打印模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsprinttemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='searchtemp')//搜索模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewssearchtemp",$gid)." set subnews='$r[subnews]',tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."',listvar='".StripAddsData($r[listvar])."',rownum='$r[rownum]',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',subtitle='$r[subtitle]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='tempvar')//公共模板变量
	{
		$sql=$empire->query("update ".GetDoTemptb("enewstempvar",$gid)." set myvar='".StripAddsData($r[tempname])."',varname='".StripAddsData($r[listvar])."',varvalue='".StripAddsData($r[temptext])."',classid='$r[classid]',isclose='$r[docode]',myorder='$r[subnews]' where varid='$r[tempid]'");
	}
	elseif($r['temptype']=='votetemp')//投票模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewsvotetemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='pagetemp')//自定义页面模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspagetemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='indexpage')//首页方案模板
	{
		$sql=$empire->query("update {$dbtbpre}enewsindexpage set tempname='".StripAddsData($r[tempname])."',temptext='".StripAddsData($r[temptext])."' where tempid='$r[tempid]'");
	}
	//公共模板
	elseif($r['temptype']=='pubindextemp')//首页模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set indextemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubcptemp')//控制面板模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set cptemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchtemp')//高级搜索表单模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchtemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchjstemp')//搜索JS模板[横向]
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchjstemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchjstemp1')//搜索JS模板[纵向]
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set searchjstemp1='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubotherlinktemp')//相关链接模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set otherlinktemp='".StripAddsData($r[temptext])."',otherlinktempsub='$r[subtitle]',otherlinktempdate='".StripAddsData($r[showdate])."' limit 1");
	}
	elseif($r['temptype']=='pubdownsofttemp')//下载地址模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set downsofttemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubonlinemovietemp')//在线播放地址模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set onlinemovietemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publistpagetemp')//列表分页模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set listpagetemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubpljstemp')//评论JS调用模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set pljstemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubdownpagetemp')//最终下载页模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set downpagetemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubgbooktemp')//留言板模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set gbooktemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publoginiframe')//登陆状态模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginiframe='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publoginjstemp')//JS调用登陆状态模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set loginjstemp='".StripAddsData($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubschalltemp')//全站搜索模板
	{
		$sql=$empire->query("update ".GetDoTemptb("enewspubtemp",$gid)." set schalltemp='".StripAddsData($r[temptext])."',schallsubnum='$r[subnews]',schalldate='".StripAddsData($r[showdate])."' limit 1");
	}
	if($sql)
	{
		//操作日志
