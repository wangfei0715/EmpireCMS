...
function AddUserpage($add,$userid,$username){
	...
	$gid=(int)$add['gid'];
	$sql=$empire->query("insert into {$dbtbpre}enewspage(title,path,pagetext,classid,pagetitle,pagekeywords,pagedescription,tempid) values('$title','$path','".addslashes($pagetext)."','$classid','".addslashes($pagetitle)."','".addslashes($pagekeywords)."','".addslashes($pagedescription)."','$tempid');");
	$id=$empire->lastid();
...
function EditUserpage($add,$userid,$username){
	...
	$tempid=(int)$add['tempid'];
	$gid=(int)$add['gid'];
	$sql=$empire->query("update {$dbtbpre}enewspage set title='$title',path='$path',pagetext='".addslashes($pagetext)."',classid='$classid',pagetitle='".addslashes($pagetitle)."',pagekeywords='".addslashes($pagekeywords)."',pagedescription='".addslashes($pagedescription)."',tempid='$tempid' where id='$id'");
	ReUserpage($id,$pagetext,$path,$title,$pagetitle,$pagekeywords,$pagedescription,$tempid);
	if($sql)
	{
		//操作日志
...
function ClearTmpFileData($add,$userid,$username){
	...
	{
		if($file=='.'||$file=='..'||$file=='test.txt'||$file=='mod'||$file=='temp'||$file=='titlepic')
		{
			continue;
		}
