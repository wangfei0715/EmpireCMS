...
<input type="file" name="titlepicfile" size="45">
  ...
  <tr> 
    ...
    <td bgcolor=ffffff>文件类型：<input name="filetype" type="text" size=10 id="filetype" value="<?=$ecmsfirstpost==1?"":DoReqValue($mid,'filetype',stripSlashes($r[filetype]))?>">
<select name="select2" onchange="document.add.filetype.value=this.value">
        <option value="">类型</option>
        <option value=".zip">.zip</option>
        <option value=".rar">.rar</option>
        <option value=".exe">.exe</option>
      </select>，文件大小：<input name="filesize" type="text" size=10 id="filesize" value="<?=$ecmsfirstpost==1?"":DoReqValue($mid,'filesize',stripSlashes($r[filesize]))?>">
