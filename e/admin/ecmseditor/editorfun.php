...
function eTranMorePic($file,$file_name,$file_type,$file_size,$add,$userid,$username){
	...
	if($add['saveurl'])
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			$sql=$empire->query("insert into {$dbtbpre}enewsfile(filename,filesize,adduser,path,filetime,classid,no,type,id,cjid,onclick,fpath) values('$r[filename]',$r[filesize],'$username','$r[filepath]','$filetime',$add[classid],'$r[filename]',$add[type],$add[filepass],$add[filepass],0,'$public_r[fpath]');");
			//缩略图
			if($add['getsmall'])
			{
				$sfiler=GetMySmallImg($add['classid'],$r[filename],$r[insertfile],$r[filepath],$r[yname],$add[swidth],$add[sheight],$r[name],$add['filepass'],$add['filepass'],$userid,$username);
...
function EditorSetTranPic($picurl,$smallpic,$add){
	...
	if($add[pic_say])
	{
		$imgstr.="<br><span style='line-height=18pt'>".htmlspecialchars($add[pic_say])."</span>";
	}
