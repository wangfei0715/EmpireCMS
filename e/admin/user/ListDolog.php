...
$query="select logid,logip,logtime,username,enews,doing from {$dbtbpre}enewsdolog";
$totalquery="select count(*) as total from {$dbtbpre}enewsdolog";
//搜索
$search='';
$where='';
if($_GET['sear']==1)
{
	...
	if($startday&&$endday)
	{
		...
		$a.="logtime<='".$endday." 23:59:59' and logtime>='".$startday." 00:00:00'";
	}
	...
	if($keyboard)
	{
		...
		elseif($show==2)
		{
			$a.=$and."logip like '%$keyboard%'";
		}
		else
		{
			$a.=$and."(username like '%$keyboard%' or logip like '%$keyboard%')";
		}
